...
- **Different providers**: Use `use-image-pull-secret` (more flexible)
...
## Cross-Provider Setup (use-image-pull-secret)

When your Kubernetes cluster and container registry are from different providers (e.g., Hetzner cluster with Harbor registry, or Hetzner cluster with GCP Artifact Registry), use the `use-image-pull-secret` approach.

### How it works

1. You create a docker-registry secret called `external-registry-secret` in the `default` namespace
2. Set `use-image-pull-secret: true` in `.devops/config/constants.yaml`
3. When you run `devops namespace create`, the tool:
   - Copies `external-registry-secret` from the `default` namespace to the new namespace
...
### Step 1: Create the external-registry-secret

The secret format depends on your registry provider:
...
### Step 2: Enable in constants.yaml
